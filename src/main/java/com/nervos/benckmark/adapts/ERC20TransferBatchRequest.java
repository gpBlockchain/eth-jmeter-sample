...
public class ERC20TransferBatchRequest extends AbstractJavaSamplerClient {
    ...
    private Integer curSendIdx = 0;

    private static AtomicInteger idxForPrivateKeys = new AtomicInteger(0);
    private static String RPC_URL = "rpcUrl";
    private static String PRIVATE_KEYS = "privateKeys";
    private static String ERC20_Address = "ERC20Address";
    private static String THREAD_SIZE = "threadSize";

    @Override
    public Arguments getDefaultParameters() {
        Arguments arguments = new Arguments();
        arguments.addArgument(RPC_URL, "1");
        arguments.addArgument(PRIVATE_KEYS, "0x723982bb19b3d9d36990fef21dbe88281bba7d67eb4ee85760d9566bcf9423d4\n" +
                "0x5aa9c3c53a68651524730ca3843314edeeebb9ef3ed55d7f08263d006a407ad0");
        arguments.addArgument(ERC20_Address, "");
        arguments.addArgument(THREAD_SIZE,"1");
        return arguments;
    }
    ...
    public void setupTest(JavaSamplerContext context) {
        ...
        String url = context.getParameter(RPC_URL);
        String privates = context.getParameter(PRIVATE_KEYS);
        //account
        this.threadSize = context.getIntParameter(THREAD_SIZE);
        this.erc20Address = context.getParameter(ERC20_Address);
        this.web3j = Web3j.build(new HttpService(url));
        try {
            this.accountList = getAccountList(privates);
        } catch (Exception e) {
            e.printStackTrace();
            System.err.println("getAccountList :{}" + e);
            return;
        }

        // 可以初始化 RPC Client
        try {
            this.bep20 = initErc20(this.accountList.get(0).getCredentials(), this.web3j, this.erc20Address);
            System.out.println("bep20 address:"+this.bep20.getContractAddress());
        } catch (Exception e) {
            e.printStackTrace();
            return;
        }
    ...
    private BEP20 initErc20(Credentials credentials, Web3j web3j, String contractAddress) throws Exception {
        if (contractAddress.equals("")) {
            RawTransactionManager cutomerTokenTxManager = TransactionUtil.getTxManage(web3j,credentials);
            return BEP20.deploy(web3j, cutomerTokenTxManager, new DefaultGasProvider()).send();
        }
        return BEP20.load(contractAddress, web3j, credentials, new DefaultGasProvider());
    }

    private List<Account> getAccountList(String rawPrivateKeys) throws Exception {
        List<Account> accountList = new ArrayList<>();
        String[] privateKeys = rawPrivateKeys.split("\n");
        Integer page = privateKeys.length / this.threadSize;
        List<Credentials> credentials = getCredentialsList(privateKeys, this.idxForPrivateKeys.getAndAdd(1), page);
        for (int i = 0; i < credentials.size(); i++) {
            Credentials credentials1 = credentials.get(i);
            BigInteger nonce = this.web3j.ethGetTransactionCount(credentials1.getAddress(), DefaultBlockParameterName.PENDING).send().getTransactionCount();
            Account account = new Account(credentials1, nonce.subtract(new BigInteger("1")));
    ...
    private List<Credentials> getCredentialsList(String[] privateKeys, Integer idx, Integer page) {
        List<Credentials> credentialsList = new ArrayList<>();
        for (int i = 0; i < privateKeys.length; i++) {
            Credentials credentials = Credentials.create(privateKeys[i]);
            credentialsList.add(credentials);
        }
        return credentialsList.subList(page * idx, (page * (1 + idx)));
    }

    @Override
    public SampleResult runTest(JavaSamplerContext javaSamplerContext) {
        String address= this.bep20.getContractAddress();
        String payload = this.bep20.transfer(address, new BigInteger("0")).encodeFunctionCall();
        Credentials fromCredentials = getCurrentCredentials();
        System.out.println("current account:"+fromCredentials.getAddress());

        SampleResult result= new SampleResult();
        result.sampleStart();
        boolean success = sendTx(this.web3j, fromCredentials, this.bep20.getContractAddress(), new BigInteger("0"), payload);
    ...
    private Credentials getCurrentCredentials() {
        while (true) {
            curSendIdx++;
            Integer currentIdx = curSendIdx % this.accountList.size();
            Account account = this.accountList.get(currentIdx);
            BigInteger nonce = null;
            try {
                nonce = this.web3j.ethGetTransactionCount(account.getCredentials().getAddress(), DefaultBlockParameterName.PENDING).send().getTransactionCount();
            } catch (IOException e) {
                System.out.println("[warn] ethGetTransactionCount failed!");
                continue;
            }
            if (nonce.compareTo(account.getLatestSendTxNonce()) > 0) {
                account.setLatestSendTxNonce(nonce);
    ...
    public static void main(String[] args) {
        Arguments arguments = new Arguments();
        arguments.addArgument(RPC_URL, "http://localhost:8024");
        arguments.addArgument(PRIVATE_KEYS,
        ...
        ERC20TransferBatchRequest sample = new ERC20TransferBatchRequest();
